...
PUBLIC_ENDPOINTS: tuple[PublicEndpoint, ...] = (
    ...
    PublicEndpoint(("GET",), "rankings/task-leaderboard/", "任务榜：按已录用任务数分页", False),
    PublicEndpoint(("GET",), "rankings/invite-leaderboard/", "邀请榜：按直接邀请人数分页", False),
    PublicEndpoint(
        ("GET",),
        "me/ranking/invite-overview/",
        "排行邀请区：累计邀请、推荐奖励/预计展示、返佣比例、邀请链接、我的排名摘要",
        True,
    ),
    PublicEndpoint(("GET",), "me/ranking/invitees/", "我的邀请明细分页（下级完成数+贡献返佣展示）", True),
    PublicEndpoint(("GET",), "me/ranking/context/", "排行底栏：我的任务/邀请排名、超越比例、总贡献 USDT", True),
